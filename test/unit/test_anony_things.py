...
def valid_advantage_address(network='mainnet'):
...
def invalid_advantage_address(network='mainnet'):    
...
def test_valid_advantage_address():
    # from dashlib import is_valid_dash_address
    from advantagelib import is_valid_advantage_address

    main = valid_advantage_address()
    test = valid_advantage_address('testnet')

    assert is_valid_advantage_address(main) is True
    assert is_valid_advantage_address(main, 'mainnet') is True
    assert is_valid_advantage_address(main, 'testnet') is False

    assert is_valid_advantage_address(test) is False
    assert is_valid_advantage_address(test, 'mainnet') is False
    assert is_valid_advantage_address(test, 'testnet') is True


def test_invalid_advantage_address():
    from advantagelib import is_valid_advantage_address

    main = invalid_advantage_address()
    test = invalid_advantage_address('testnet')

    assert is_valid_advantage_address(main) is False
    assert is_valid_advantage_address(main, 'mainnet') is False
    assert is_valid_advantage_address(main, 'testnet') is False

    assert is_valid_advantage_address(test) is False
    assert is_valid_advantage_address(test, 'mainnet') is False
    assert is_valid_advantage_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    ...
    from advantagelib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % advantagelib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(advantagelib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(advantagelib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(advantagelib.blocks_to_seconds(16616)) == 2612035
