...
def is_valid_advantage_address(address, network='mainnet'):
    ...
    advantage_version = 28 if network == 'testnet' else 5
    ...
    try:
        ...
        print('advantageversion:')
        print(advantage_version)
    except:
        # rescue from exception, not a valid Dash address
        return False

    # if (address_version != dash_version):
    if (address_version != advantage_version):
        return False
...
def SHIM_serialise_for_advantaged(sentinel_hex):
    from models import ADVANTAGED_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = ADVANTAGED_GOVOBJ_TYPES[govtype]
    ...
    # re-pack
    advantaged_hex = serialise(obj)
    return advantaged_hex


# shims 'til we can fix the dashd side
def SHIM_deserialise_from_advantaged(advantaged_hex):
    from models import ADVANTAGED_GOVOBJ_TYPES

    # unpack
    obj = deserialise(advantaged_hex)
...
def did_we_vote(output):
    ...
    _, conf_filename = os.path.split(config.advantage_conf)
    try:
