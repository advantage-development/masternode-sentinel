...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, advantaged):
        golist = advantaged.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_advantaged(advantaged, item)
    ...
    @classmethod
    def import_gobject_from_advantaged(self, advantaged, rec):
        import decimal
        import advantagelib
        import inflection
        ...
        # shim/advantaged conversion
        object_hex = advantagelib.SHIM_deserialise_from_advantaged(object_hex)
        objects = advantagelib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from advantaged...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from advantaged, with every run
        ...
        # get/create, then sync payment amounts, etc. from advantaged - advantaged is the master
        try:
            ...
            printdbg("Got invalid object from advantaged! %s" % e)
            govobj.vote_delete(advantaged)
            return (govobj, None)
    ...
    def vote_delete(self, advantaged):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(advantaged, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, advantaged, signal, outcome):
        import advantagelib
        ...
        output = advantaged.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = advantagelib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(advantaged, signal)

    def sync_network_vote(self, advantaged, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = advantaged.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = ADVANTAGED_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            if not advantagelib.is_valid_advantage_address(self.payment_address, config.network):
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (advantagelib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/advantagedrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import advantagelib
        obj_data = advantagelib.SHIM_serialise_for_advantaged(self.serialise())
    ...
    def prepare(self, advantaged):
        try:
            object_hash = advantaged.rpc_command(*self.get_prepare_command())
...
#     govobj_type = ADVANTAGED_GOVOBJ_TYPES['superblock']
...
#             if not advantagelib.is_valid_dash_address(addr, config.network):
...
#     def hash(self):
#         import advantagelib
#         return advantagelib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    # govobj_type = advantaged_GOVOBJ_TYPES['watchdog']
    govobj_type = ADVANTAGED_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    # def active(self, advantaged):
    def active(self, advantaged):
        now = int(time.time())
        resultset = self.select().where(
            # self.created_at >= (now - advantaged.SENTINEL_WATCHDOG_MAX_SECONDS)
            self.created_at >= (now - advantaged.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    # def expired(self, advantaged):
    def expired(self, advantaged):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - advantaged.SENTINEL_WATCHDOG_MAX_SECONDS)
    ...
    # def is_expired(self, advantaged):
    def is_expired(self, advantaged):
        now = int(time.time())
        return (self.created_at < (now - advantaged.SENTINEL_WATCHDOG_MAX_SECONDS))

    # def is_valid(self, advantaged):
    def is_valid(self, advantaged):
        if self.is_expired(advantaged):
            return False

        return True

    # def is_deletable(self, advantaged):
    def is_deletable(self, advantaged):
        if self.is_expired(advantaged):
            return True
