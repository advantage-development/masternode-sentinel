...
sentinel_cfg = AdvantageConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.2"
# min_dashd_proto_version_with_sentinel_ping = 70207
min_advantaged_proto_version_with_sentinel_ping = 200000


# def get_dash_conf():
def get_advantage_conf():
    home = os.environ.get('HOME')

    # dash_conf = os.path.join(home, ".dashcore/dash.conf")
    # TEMPORARY UNTIL WE SWITCH OVER TO ADVANTAGE PATHS
    advantage_conf = os.path.join(home, ".advantage/advantage.conf")
    if sys.platform == 'darwin':
        # dash_conf = os.path.join(home, "Library/Application Support/DashCore/dash.conf")
        advantage_conf = os.path.join(home, "Library/Application Support/Anon/advantage.conf")

    # dash_conf = sentinel_cfg.get('dash_conf', dash_conf)
    advantage_conf = sentinel_cfg.get('advantage_conf', advantage_conf)

    # return dash_conf
    return advantage_conf
...
advantage_conf = get_advantage_conf()
network = get_network()
db = get_db_conn()
