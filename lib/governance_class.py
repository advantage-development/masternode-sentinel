...
class GovernanceClass(object):
    ...
    def vote(self, advantaged, signal, outcome):
        return self.go.vote(advantaged, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, advantaged):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(advantaged, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(advantaged, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to advantaged "gobject list" output
    ...
    def advantaged_serialise(self):
        import advantagelib
        return advantagelib.SHIM_serialise_for_advantaged(self.serialise())
